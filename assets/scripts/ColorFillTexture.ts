...
export default class ColorFilterTexture extends Component {
    ...
    private setColor(color: Color, x: number, y: number): boolean {
        // 修正越界判断，像素索引有效区间为 [0, width-1] 和 [0, height-1]
    ...
    // 判断像素坐标是否在图片范围内
    public static isInside(width: number, height: number, x: number, y: number): boolean {
    ...
    public fillTextureColor(color: Color, x: number, y: number): void {
        ...
        y = ColorFilterTexture.translateY(this.height, y);
        // 若起点越界则直接返回
        if (!ColorFilterTexture.isInside(this.width, this.height, x, y)) return;
    ...
    private async fillColor(color: Color, x: number, y: number, gridIndexTypes: readonly GridIndex[]) {
        // visited 按像素记录，避免重复入队造成死循环
        const visited = new Uint8Array(this.width * this.height);
        const pushIfValid = (list: any[], px: number, py: number, neigh: readonly GridIndex[]) => {
            if (!ColorFilterTexture.isInside(this.width, this.height, px, py)) return;
            const pid = px + py * this.width;
            if (visited[pid]) return;
            visited[pid] = 1;
            list.push({x: px, y: py, neighborTypes: neigh});
        };

        let colorPointList1: any[] = [];
        pushIfValid(colorPointList1, x, y, gridIndexTypes);
        let colorPointList2: any[] = [];
        let fillCount = 0;
        do {
